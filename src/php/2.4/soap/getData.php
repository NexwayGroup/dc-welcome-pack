...
/**
 * Sample SOAP client for querying for order details on Digital Content SOAP interface v2.4
 *
 * Use on the command line the following way (Linux): `php getData.php secret orderNumber`
...
// Send "getData" request with defined payload
fprintf(STDOUT, "Requesting information for order %s\n", ORDER_NUMBER);
try {
    $response = $soap->getData($payload);
} catch (Exception $e) {
