 ...
 *
 * Use on the command line the following way (Linux): `php getDownloadInfo.php secret orderNumber`
 * where "secret" is the key given to you by Nexway onboarding team
 * and "orderNumber" is the partner's (your) order number as passed at "getDownloadInfo" call.
 *
...
// Send "getDownloadInfo" request with defined payload
fprintf(STDOUT, "Requesting download information on order %s\n", ORDER_NUMBER);
try {
    $response = $soap->getDownloadInfo($payload);
} catch (Exception $e) {
